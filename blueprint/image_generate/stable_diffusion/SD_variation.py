## from https://github.com/Stability-AI/stablediffusion/blob/main/scripts/streamlit/stableunclip.py
...
def get_interactive_image(img_path,key=None):
    image = img_path
...
def load_img(img_path,display=False, key=None):
    image = get_interactive_image(img_path,key=key)
    if display:
        st.image(image)
    w, h = image.size
...
def get_init_img(img_path,batch_size=1, key=None):
    init_image = load_img(img_path,key=key).cuda()
...
#@st.cache(allow_output_mutation=True, suppress_st_warning=True) <-- check this lineggfvcf
def init(version, load_karlo_prior):
    state = dict()
    if not "model" in state:
        ...
        if load_karlo_prior:
            ...
            print("Loading KARLO CLIP prior...")
...
def parse_config():
    ...
    parser.add_argument('--model', type=str, default="Stable unCLIP-L", required=False, help='choose Stable unCLIP-L, Stable unOpenCLIP-H, Full karlo. defulat model is Stable unCLIP-L')
    parser.add_argument('--input_dir', type=str, default='./outputs/SD_result', required=False, help ='Enter your input data path. If your data set size is not 1, you have to set --noise True')
    parser.add_argument('--use_karlo_prior', type=str, default=False, help='Use karlo prior or not')
    parser.add_argument('--prompt', type=str, default='a professional photograph', required=False, help='Enter your prompt')
    parser.add_argument('--negative_prompt', type=str, default="", help='Enter your negative prompt')
    parser.add_argument('--scale', type=int, default=10, help='enter scale factor')
    parser.add_argument('--num_rows', type=int, default=2, help='Enter number of rows')
    parser.add_argument('--num_cols', type=int, default=2, help='Enter number of cols')
    parser.add_argument('--step', type=int, default=20, help='Enter step size')
    parser.add_argument('--DDIM', type=int, default=0, help='Enter DDIM value')
    parser.add_argument('--force_fp32', default=False, help='Enter False or True')
    parser.add_argument('--sampler', default='DDIM', type = str, help='Choose DPM, DDIM. If user_karlo_prior is not false. If user_karlo_prior is False, do not enter')
    parser.add_argument('--noise', default=False, help='Apply noise to embedding mix. Default value is False')
...
if __name__ == "__main__":
    ...
    args = parse_config()
    use_karlo_prior = args.use_karlo_prior
    version = args.model
    state = init(version=version, load_karlo_prior=args.use_karlo_prior)
    prompt = args.prompt
    negative_prompt = args.negative_prompt
    scale = args.scale
    number_rows = args.num_rows
    number_cols = args.num_cols
    steps = args.step
    eta = args.DDIM
    sampler = args.sampler
    apply_noise = args.noise
    ori_direct = args.input_dir
    force_full_precision = args.force_fp32# TODO: check if/where things break.
    if version != "Full Karlo":
        H = VERSION2SPECS[version]["H"]
        W = VERSION2SPECS[version]["W"]
        C = VERSION2SPECS[version]["C"]
        f = VERSION2SPECS[version]["f"]

    SAVE_PATH = os.path.join(SAVE_PATH, version)
    path = os.path.join(SAVE_PATH, "samples")
    ...
    if use_karlo_prior:
        ...
        num_inputs = len([(os.path.join(ori_direct, f)) for f in os.listdir(ori_direct)])


        def make_conditionings_from_input(input_path, num=1, key=None):
            init_img = get_init_img(input_path, batch_size=number_cols, key=key)
            with torch.no_grad():
        ...
        adm_inputs = list()
        weights = list()
        img_path = [(os.path.join(ori_direct, f)) for f in os.listdir(ori_direct)]
        for n in range(num_inputs):
            adm_cond, adm_uc, w = make_conditionings_from_input(num=n + 1, key=n, input_path = str(img_path[n]))
            weights.append(w)
    ...
    #    result = st.empty()
        ...
        for path_ in ori_path:
            original = str(path_)
            name = original.split('.')[-2].split('/')[-2]
            extension = original.split('.')[-1]
            print(name)
            print(extension)
            new_path = path + '/' + name + '.' + extension
            print(original)
            print(new_path)
